...
from converter import extract_text_pypdf, convert_page
...
def test_extract_text_pypdf_mocked(tmp_path):
    ...
    with patch("pypdf.PdfReader", return_value=mock_reader):
        text, error = extract_text_pypdf(str(dummy_pdf), 1)
...
def test_extract_text_pypdf_out_of_range(tmp_path):
    ...
    with patch("pypdf.PdfReader", return_value=mock_reader):
        text, error = extract_text_pypdf(str(dummy_pdf), 5)
        assert text is None
...
def test_convert_page_success():
    ...
    with (patch("converter.extract_text_pypdf", return_value=(longer_text, None)),
        ...
        data, error = convert_page("dummy.pdf", 1)
